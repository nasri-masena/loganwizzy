...
# New configs
SHORT_BUY_SELL_DELAY = float(os.getenv("SHORT_BUY_SELL_DELAY", "0.3"))  # wait after buy before placing limit sell
HOLD_THRESHOLD_HOURS = float(os.getenv("HOLD_THRESHOLD_HOURS", "4.0"))  # if any active pos older than this -> block buys
...
TEMP_SKIP = {}
SKIP_SECONDS_ON_MARKET_CLOSED = 60
...
notify = send_telegram  # alias used by new functions
...
# -------------------------
# Binance trading helpers & client init
# -------------------------
...
# -------------------------
# helpers: format, ceil_step, get_free_asset, has_stale_positions
# -------------------------
...
# -------------------------
# place_limit_sell_strict
# -------------------------
def place_limit_sell_strict(symbol, qty, sell_price, retries=5, delay=0.8):
    """
    Robust LIMIT SELL with retries + adjustments + cancel open SELLs + final fallback.
    """
    try:
        client = init_binance_client()
        if not client:
...
# -------------------------
# Execute trade (uses place_limit_sell_strict + delay)
# -------------------------
def execute_trade(chosen):
    ...
    try:
        ...
        # WAIT a short delay to let balances settle BEFORE placing limit sell
        time.sleep(SHORT_BUY_SELL_DELAY)

        # place robust limit sell
        try:
            sell_price = avg_price * (1.0 + (LIMIT_PROFIT_PCT/100.0))
            sell_resp = place_limit_sell_strict(symbol, executed_qty, sell_price)
            if sell_resp:
                # determine used price if available
                sell_price_used = None
                try:
                    sell_price_used = float(sell_resp.get("price") or sell_resp.get("orderId") and sell_price)
                except Exception:
                    sell_price_used = sell_price
                send_telegram(f"💰 LIMIT SELL initiated: `{symbol}` Qty `{executed_qty}` @ `{sell_price}` (+{LIMIT_PROFIT_PCT}%)")
                with RECENT_BUYS_LOCK:
                    RECENT_BUYS[symbol].update({"qty": executed_qty, "buy_price": avg_price, "sell_price": sell_price, "order_raw": sell_resp})
                    persist_recent_buys()
                return True
            else:
                # If limit placement failed, attempt market fallback
...
# -------------------------
# pick_coin (checks stale positions before buying)
# -------------------------
def pick_coin():
    # Block new buys if there is any stale/old active position beyond threshold
    if has_stale_positions():
        notify("⚠️ Buy blocked: found active position older than threshold hours. Skipping buy cycle.")
        return None
    ...
    for t in tickers:
        ...
        with RECENT_BUYS_LOCK:
            ...
            if last_buy and now < last_buy.get("ts", 0) + BUY_LOCK_SECONDS:
                continue
            # also skip if symbol currently reserved
            if last_buy and not last_buy.get("closed") and last_buy.get("reserved"):
                continue
