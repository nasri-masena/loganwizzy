# signal_bot_with_monitor_fixed.py
...
LIMIT_SELL_RETRIES = int(os.getenv("LIMIT_SELL_RETRIES", "3"))  # reduced to 3 by request
...
def place_limit_sell_strict(symbol, qty, sell_price, retries=None, delay=0.8):
    ...
    retries default set from LIMIT_SELL_RETRIES.
    """
    if retries is None:
...
# ---------- execute_trade (stores qty & ts immediately) ----------
def execute_trade(chosen):
    ...
    if not client:
        ...
        with RECENT_BUYS_LOCK:
            RECENT_BUYS.pop(symbol,None); persist_recent_buys()
        return False
    try:
        ...
        if sell_resp:
            ...
            with RECENT_BUYS_LOCK: 
                # mark closed if fallback succeeded/failed; we remove reserved pos so bot can resume
                RECENT_BUYS.pop(symbol,None); persist_recent_buys()
            return False
    except BinanceAPIException as e:
