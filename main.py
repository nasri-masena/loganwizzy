...
def monitor_and_move_stop_to_breakeven(symbol, entry_price, qty, f,
    ...
    try:
        trigger_pct = trigger_pct or globals().get('BREAKEVEN_TRIGGER_PCT', 0.008)  # 0.8%
        buffer_pct = buffer_pct or globals().get('BREAKEVEN_BUFFER_PCT', 0.002)
        monitor_seconds = monitor_seconds or globals().get('BREAKEVEN_MONITOR_SECONDS', 3600)
        poll_interval = poll_interval or globals().get('BREAKEVEN_POLL_INTERVAL', 3)
        ...
        deadline = time.time() + float(monitor_seconds)

        while time.time() < deadline:
            try:
                p = get_price(symbol)  # use live price instead of cached
                if not p:
                    time.sleep(poll_interval)
                    continue

                if p >= entry_price * (1 + trigger_pct):
                    sl_target_price = _ceil_to_tick(entry_price * (1 + buffer_pct), tick) if tick else entry_price * (1 + buffer_pct)
                    qty_to_place = round_step(qty, step) if step else qty

                    if qty_to_place <= 0 or (min_qty and qty_to_place < min_qty):
                        notify(f"⚠️ Breakeven skip {symbol}: qty too small.")
                        return False

                    if min_notional and qty_to_place * sl_target_price < min_notional * 0.9:
                        notify(f"⚠️ Breakeven skip {symbol}: below minNotional.")
                        return False

                    try:
                        cancel_all_open_orders(symbol)
                    except Exception as e:
                        notify(f"⚠️ Could not cancel old orders for {symbol}: {e}")

                    limit_price = _floor_to_tick(sl_target_price * 0.999, tick)
                    client.create_order(
                        symbol=symbol,
                        side="SELL",
                        type="STOP_LOSS_LIMIT",
                        stopPrice=format_price(sl_target_price, f.get('tickSize')),
                        price=format_price(limit_price, f.get('tickSize')),
                        timeInForce='GTC',
                        quantity=format_qty(qty_to_place, step)
                    )
                    notify(f"🔒 Moved SL for {symbol} → {sl_target_price:.6f} (breakeven mode)")
                    return True

            except Exception as e:
                notify(f"⚠️ monitor_and_move_stop_to_breakeven loop error: {e}")

            time.sleep(poll_interval)

    except Exception as e:
        notify(f"⚠️ monitor_and_move_stop_to_breakeven fatal error: {e}")
        ...
        try:
            ...
            try:
                ...
                while time.time() < monitor_deadline:
                    ...
                    # =============== FIXED SECTION (Confirmation Delay Added) ===============
                    if free_after is not None:
                        ...
                        # Check if likely sold, but confirm several times
                        if approx_sold >= max(min_qty, qty * 0.999):
                            confirm_ok = False
                            for _ in range(3):  # ~9s confirmation delay
                                time.sleep(3)
                                check_orders = get_open_orders_cached(symbol) or []
                                check_free = get_free_asset(asset)
                                if check_free is not None:
                                    diff = max(0.0, qty - check_free)
                                    if diff >= max(min_qty, qty * 0.999) and not check_orders:
                                        confirm_ok = True
                                        break
                            if confirm_ok:
                                sold_detected = True
                ...
                try:
                    if sold_detected and sold_qty and exit_price:
                        ...
                        try:
                            ...
                            _update_metrics_for_profit(total_profit, picked_symbol=symbol, was_win=was_win)
