...
def monitor_and_roll(symbol, qty, entry_price, f):
    try:
        orig_qty = float(qty)
        if orig_qty <= 0:
            notify(f"⚠️ monitor_and_roll: invalid orig_qty {orig_qty} for {symbol}")
            return False, entry_price, 0.0, 0.0

        # config + helpers
        sleep_between = float(globals().get('SLEEP_BETWEEN_CHECKS', SLEEP_BETWEEN_CHECKS))
        roll_on_rise_pct = float(globals().get('ROLL_ON_RISE_PCT', ROLL_ON_RISE_PCT))
        roll_trigger_delta_abs = float(globals().get('ROLL_TRIGGER_DELTA_ABS', ROLL_TRIGGER_DELTA_ABS))
        roll_tp_step_abs = float(globals().get('ROLL_TP_STEP_ABS', ROLL_TP_STEP_ABS))
        roll_sl_step_abs = float(globals().get('ROLL_SL_STEP_ABS', ROLL_SL_STEP_ABS))
        roll_cooldown = float(globals().get('ROLL_COOLDOWN_SECONDS', ROLL_COOLDOWN_SECONDS))
        max_rolls = int(globals().get('MAX_ROLLS_PER_POSITION', MAX_ROLLS_PER_POSITION))
        failed_threshold = int(globals().get('FAILED_ROLL_THRESHOLD', FAILED_ROLL_THRESHOLD))
        failed_skip = int(globals().get('FAILED_ROLL_SKIP_SECONDS', FAILED_ROLL_SKIP_SECONDS))
        quote = globals().get('QUOTE', QUOTE)
        ...
        # local state
        curr_tp = entry_price * (1 + globals().get('BASE_TP_PCT', BASE_TP_PCT) / 100.0)
        curr_sl = entry_price * (1 - globals().get('BASE_SL_PCT', BASE_SL_PCT) / 100.0)
        # round to ticks
        def ceil_to_tick(v, t): return (math.ceil(v / t) * t) if t and t > 0 else v
        def floor_to_tick(v, t): return (math.floor(v / t) * t) if t and t > 0 else v

        curr_tp = ceil_to_tick(curr_tp, tick)
        curr_sl = floor_to_tick(curr_sl, tick)
        ...
        notify(f"🔍 monitor_and_roll started for {symbol}: entry={entry_price:.8f}, qty={orig_qty:.8f}, TP={curr_tp:.8f}, SL={curr_sl:.8f}")

        while True:
            try:
                time.sleep(sleep_between)

                # fetch latest price (prefer cached fast method)
                p = None
                try:
                ...
                if p is None:
                    try:
                        # last resort
                        res = safe_api_call(get_client().get_symbol_ticker, symbol=symbol)
                        p = float(res.get('price') or res.get('lastPrice') or 0.0) if res else None
                    except Exception:
                        p = None

                if p is None:
                    notify(f"⚠️ monitor_and_roll: couldn't fetch price for {symbol}, continuing.")
                    continue
                ...
                # compute available for sell (floor to stepSize and cap to orig_qty)
                free_qty = get_free_asset(asset)
                # floor free_qty to step
                avail_floor = 0.0
                if step and step > 0:
                    avail_floor = math.floor(free_qty / step) * step
                else:
                    avail_floor = free_qty
                available_for_sell = min(avail_floor, orig_qty)

                # detect closed position: no open orders and not enough free asset to sell -> assume closed
                open_orders = get_open_orders_cached(symbol)
                if available_for_sell < max(min_qty, orig_qty * 0.02) and (not open_orders or len(open_orders) == 0):
                    ...
                    return True, exit_price, profit_usd, 0.0

                # roll trigger conditions:
                #  - price moved up by configured pct from entry, OR
                #  - price is near current TP (within TRIGGER_PROXIMITY)
                rise_trigger_pct = price_now >= entry_price * (1 + roll_on_rise_pct / 100.0)
                near_tp_trigger = price_now >= curr_tp * (1 - float(globals().get('TRIGGER_PROXIMITY', TRIGGER_PROXIMITY)))
                moved_enough_abs = (price_now - last_roll_price) >= roll_trigger_delta_abs
                now_ts = time.time()
                can_roll_time = (now_ts - last_roll_ts) >= roll_cooldown
                passed_price_debounce = price_now >= (last_roll_price + roll_trigger_delta_abs)

                should_roll = (rise_trigger_pct or near_tp_trigger) and price_moving_up

                if should_roll and passed_price_debounce and can_roll_time and available_for_sell >= min_qty:
                    if roll_count >= max_rolls:
                        notify(f"⚠️ Reached max rolls ({max_rolls}) for {symbol}; will not roll further.")
                        last_roll_ts = now_ts
                        continue

                    notify(f"🔎 Roll triggered for {symbol}: price={price_now:.8f}, entry={entry_price:.8f}, curr_tp={curr_tp:.8f}, avail={available_for_sell:.8f}")
                    ...
                    new_tp = ceil_to_tick(candidate_tp, tick)
                    new_sl = floor_to_tick(candidate_sl, tick)
                    # ensure sensible spacing
                    if new_tp <= new_sl + (tick or 0.0):
                        if tick and tick > 0:
                            new_tp = new_sl + tick * 2
                        else:
                            new_tp = new_sl + max(1e-8, roll_tp_step_abs)

                    if new_tp <= curr_tp:
                        new_tp = ceil_to_tick(curr_tp + (tick or roll_tp_step_abs), tick)

                    # compute initial sell_qty based on available_for_sell
                    sell_qty = math.floor(available_for_sell / (step or 1e-8)) * (step or 1e-8)
                    if sell_qty <= 0 or sell_qty < min_qty:
                        notify(f"⚠️ Roll skipped: sell_qty {sell_qty} too small or < minQty.")
                        last_roll_ts = now_ts
                        continue

                    # MIN_NOTIONAL handling: try bump TP first, then increase qty if possible
                    if min_notional:
                        notify(f"🔎 Roll diagnostics for {symbol}: sell_qty={sell_qty:.8f}, candidate_tp={new_tp:.8f}, minNotional={min_notional:.8f}")
                        # try bumping TP up to a limited number of ticks
                        adj_cnt = 0
                        max_adj = 50
                        while adj_cnt < max_adj and sell_qty * new_tp < min_notional - 1e-12:
                            if tick and tick > 0:
                                new_tp = ceil_to_tick(new_tp + tick, tick)
                            else:
                                new_tp = new_tp + max(1e-8, new_tp * 0.001)
                            adj_cnt += 1
                        if sell_qty * new_tp >= min_notional - 1e-12:
                            notify(f"ℹ️ Rolled TP bumped to {new_tp:.8f} to meet minNotional.")
                        else:
                            # compute needed qty for the bumped TP
                            needed_qty = ceil_step(min_notional / new_tp, step or 1e-8)
                            free_qty_now = get_free_asset(asset)
                            notify(f"ℹ️ After TP bumps can't meet minNotional: needed_qty={needed_qty:.8f}, available_for_sell={available_for_sell:.8f}, free_qty={free_qty_now:.8f}")
                            if needed_qty <= available_for_sell + 1e-12:
                                notify(f"ℹ️ Increasing sell_qty to {needed_qty:.8f} to meet minNotional.")
                                sell_qty = needed_qty
                            else:
                                # cannot meet minNotional safely
                                notify(f"⚠️ Roll aborted: cannot meet minNotional for {symbol} even after TP bumps. needed_qty={needed_qty:.8f}")
                                # fallback decision: increment fail counter and skip (safer)
                    ...
                    # final rounding to step/tick before placing
                    sell_qty = math.floor(sell_qty / (step or 1e-8)) * (step or 1e-8)
                    qty_str = format_qty(sell_qty, step)
                    tp_str = format_price(new_tp, tick)
                    sp_str = format_price(new_sl, tick)
                    sl_price_for_order = format_price(new_sl - (tick or 0.0), tick)

                    # Cancel existing open orders then place OCO
                    last_roll_ts = now_ts
                    ...
                    oco_res = None
                    try:
                        oco_res = place_oco_sell(symbol, sell_qty, entry_price, explicit_tp=new_tp, explicit_sl=new_sl)
                    except Exception as e:
                        notify(f"⚠️ place_oco_sell exception during roll for {symbol}: {e}")
                        oco_res = None
                    ...
                    if oco_res:
                        ...
                        notify(f"⚠️ Roll attempt failed to place OCO for {symbol}; attempting fallback placement.")
                        time.sleep(0.3)
                        fallback = place_oco_sell(symbol, sell_qty, entry_price, tp_pct=globals().get('BASE_TP_PCT', BASE_TP_PCT), sl_pct=globals().get('BASE_SL_PCT', BASE_SL_PCT))
                        if fallback:
                            notify("ℹ️ Fallback OCO placed after failed roll attempt.")
                            last_roll_price = price_now
                        else:
                ...
                err = str(be)
                notify(f"⚠️ BinanceAPIException in monitor_and_roll: {err}")
                if '-1003' in err or 'Too much request weight' in err:
                    prev = RATE_LIMIT_BACKOFF if RATE_LIMIT_BACKOFF else RATE_LIMIT_BASE_SLEEP
                    RATE_LIMIT_BACKOFF = min(prev * 2 if prev else RATE_LIMIT_BASE_SLEEP, RATE_LIMIT_BACKOFF_MAX)
                    notify(f"❗ Rate-limit detected — backing off {RATE_LIMIT_BACKOFF}s and TEMP skipping {symbol}.")
                    TEMP_SKIP[symbol] = time.time() + RATE_LIMIT_BACKOFF
