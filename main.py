...
APPROACH_PCT_BEFORE_CANCEL = 0.995
TRAILING_INCREMENT_PCT = 0.45
TRAILING_SELL_DROP_PCT = 0.4
TRAILING_MIN_RISE_PCT = 0.12
TRAILING_START_DELAY = 0.6
TRAILING_POLL_INTERVAL = 0.8
MIN_TIME_BETWEEN_TRAILING_ACTIONS = 1.0
TRAILING_RETRY_INTERVAL = 10.0
MAX_TRAILING_RETRIES = 5
TRAILING_STEP_MIN = 0.0002
TRAILING_STEP_PCT_MIN = 0.1

SMART_SELL_ENABLED = True
SMART_SELL_CONSECUTIVE_DOWN = 2
SMART_SELL_MIN_CUM_DROP_PCT = 0.6
SMART_SELL_VOL_MULTIPLIER = 1.1
SMART_SELL_OB_ASKS_OVER_BIDS = 1.08
SMART_SELL_MIN_LAST_DROP_FROM_BUY = 0.3
...
def place_limit_sell_strict(
    ...
    try:
        ...
        def _free_reserved_qty_and_open_orders():
            reserved = 0.0
            try:
...
def soft_fallback_sell(symbol, qty, last_price):
    # try a tight limit slightly below current last_price to reduce slippage
    try:
        target = float(last_price) * 0.999  # 0.1% below last price (aggressive buy-side)
        # round to tick
        target_str = format_price_for_symbol(target, symbol)
    ...
    # attempt one aggressive limit placement (short wait), then fallback to market
    resp = place_limit_sell_strict(symbol, qty, target_price, retries=1, delay=0.2)
    if resp:
        # give tiny time to fill
        time.sleep(0.6)
        # check if filled
        client = init_binance_client()
        if client:
            try:
                ...
                if isinstance(resp, dict):
                    ...
                    oid = getattr(resp, "orderId", None) or getattr(resp, "clientOrderId", None)
                if oid:
                    filled = wait_for_order_fill(client, symbol, oid, timeout=1, poll=0.4)
                    if filled and (filled.get("status") or "").upper() == "FILLED":
                        return filled
            except Exception:
                pass
    # last resort: market sell
    return cancel_then_market_sell(symbol, qty)
        ...
        while True:
            ...
            try:
                if highest and (last_price <= (float(highest) * (1.0 - (float(TRAILING_SELL_DROP_PCT) / 100.0)))):
                    ...
                    resp = soft_fallback_sell(symbol, qty, last_price)
                    if resp:
                        add_blacklist(symbol)
