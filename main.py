...
ORDERBOOK_MIN_IMBALANCE = 1.10

BREAKEVEN_TRIGGER_PCT = 0.006
BREAKEVEN_BUFFER_PCT = 0.0015
BREAKEVEN_MONITOR_SECONDS = 60 * 20
...
def pick_coin():
    ...
    BLACKLISTED_TOKENS = [
        "ASRUSDT", "ATMUSDT", "PSGUSDT", "PORTOUSDT", "JUVUSDT", "BARUSDT", "CITYUSDT"
    ]
    BLACKLISTED_KEYWORDS = ["ASR", "PORTO", "ATM", "PSG", "JUV", "BAR", "CITY"]
    ...
    try:
        ...
        REQUEST_SLEEP = float(globals().get('REQUEST_SLEEP', 0.3))
        KLINES_LIMIT = int(globals().get('KLINES_LIMIT', 5))
        REQUIRE_OB_IN_PICK = bool(globals().get('REQUIRE_ORDERBOOK_BEFORE_BUY', True))
        PREBUY_BREAKOUT_MARGIN = float(globals().get('PREBUY_BREAKOUT_MARGIN', 0.005))
        ...
        EMA_MIN = float(globals().get('EMA_UPLIFT_MIN_PCT', globals().get('EMA_UPLIFT_MIN', 0.0015)))
        SCORE_MIN = float(globals().get('SCORE_MIN_THRESHOLD', 11.5))
        ...
        for sym, last_price, qvol, change_pct in sampled:
            try:
                ...
                try:
                    ...
                    score += 0.0
                score += max(0.0, (vol_ratio - 1.0)) * 12.0
                score += ema_uplift * 120.0
...
def monitor_partial_tp(symbol, part_qty, tp_price, timeout=600):
    deadline = time.time() + timeout
    f_local = get_filters(get_symbol_info_cached(symbol))
    asset_local = symbol[:-len(QUOTE)]
    order_filled = False
    while time.time() < deadline:
        time.sleep(5)
        open_orders = get_open_orders_cached(symbol) or []
        tp_order_present = any(
            o.get('side') == 'SELL'
            and float(o.get('price', 0)) == float(format_price(tp_price, f_local.get('tickSize')))
            and float(o.get('origQty', 0)) == float(format_qty(part_qty, f_local.get('stepSize')))
            for o in open_orders
        )
        if not tp_order_present:
            order_filled = True
            break
    if not order_filled:
        notify(f"⚠️ Partial TP not filled after {timeout}s, attempting market sell for partial qty.")
        try:
            place_market_sell_fallback(symbol, part_qty, f_local)
        except Exception as e:
            notify(f"❌ Market sell fallback for partial TP failed: {e}")
            
def monitor_position_close(symbol, qty, entry_price, step, min_qty, asset, monitor_deadline):
    sold_detected = False
    sold_qty = 0.0
    exit_price = None
    while time.time() < monitor_deadline:
        ...
        # Only consider position closed if ALL orders are gone AND asset is sold
        if not open_orders and approx_sold >= max(min_qty, qty * 0.999):
            sold_detected = True
            ...
            break
        time.sleep(3)
    # Final notification when position truly closed
    if sold_detected and sold_qty and exit_price:
        total_profit = (exit_price - entry_price) * sold_qty
        try:
        ...
        try:
            ...
            if globals().get('NOTIFY_ON_CLOSE', True):
                notify_position_closed(symbol, exit_price, total_profit, micro_sold_qty=0, filled_qty=sold_qty)
        except Exception as e:
...
def trade_cycle():
    ...
    while True:
        try:
            ...
            # If API banned, sleep a bit and continue
            ...
            partial_tp_thread = None
            try:
                if globals().get('PARTIAL_TP_ENABLED', True):
                    part_qty = qty * float(globals().get('PARTIAL_TP_FRACTION', 0.25))
                    tp_price = entry_price * (1 + float(globals().get('PARTIAL_TP_PCT', PARTIAL_TP_PCT)))
                    def monitor_partial_tp(symbol, part_qty, tp_price, timeout=600):
                        deadline = time.time() + timeout
                        f_local = get_filters(get_symbol_info_cached(symbol))
                        ...
                        if not order_filled:
                            ...
                            try:
                                ...
                                notify(f"❌ Market sell fallback for partial TP failed: {e}")
                    partial_tp_thread = threading.Thread(target=monitor_partial_tp, args=(symbol, part_qty, tp_price, 600), daemon=True)
                    partial_tp_thread.start()
            ...
            try:
                ...
                monitor_position_close(symbol, qty, entry_price, step, min_qty, asset, monitor_deadline)
