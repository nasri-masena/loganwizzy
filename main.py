...
MIN_OB_LIQUIDITY = 4000.0      # require reasonable bid quote liquidity
...
KLINES_1M_LIMIT = int(os.getenv("KLINES_1M_LIMIT", "4"))
EMA_SHORT = int(os.getenv("EMA_SHORT", "3"))
EMA_LONG = int(os.getenv("EMA_LONG", "10"))
RSI_PERIOD = int(os.getenv("RSI_PERIOD", "14"))

# ORDERBOOK / VOLUME thresholds
OB_DEPTH = int(os.getenv("OB_DEPTH", "5"))
MIN_OB_IMBALANCE = float(os.getenv("MIN_OB_IMBALANCE", "1.08"))
MAX_OB_SPREAD_PCT = float(os.getenv("MAX_OB_SPREAD_PCT", "1.0"))
MIN_VOL_RATIO = float(os.getenv("MIN_VOL_RATIO", "1.6"))
MIN_VOL_RATIO_1M = float(os.getenv("MIN_VOL_RATIO_1M", "1.25"))
MIN_EMA_LIFT = float(os.getenv("MIN_EMA_LIFT", "0.0010"))

# PICKER / POOL
TOP_EVAL_RANDOM_POOL = int(os.getenv("TOP_EVAL_RANDOM_POOL", "40"))
TOP_BY_24H_VOLUME = int(os.getenv("TOP_BY_24H_VOLUME", "80"))
FINAL_CHOICES = int(os.getenv("FINAL_CHOICES", "2"))
...
# runtime / pacing
TRADE_USD = float(os.getenv("TRADE_USD", "10.0"))
SLEEP_BETWEEN_CHECKS = int(os.getenv("SLEEP_BETWEEN_CHECKS", "30"))
CYCLE_DELAY = int(os.getenv("CYCLE_DELAY", "15"))
COOLDOWN_AFTER_EXIT = int(os.getenv("COOLDOWN_AFTER_EXIT", "10"))

# order / protection
TRIGGER_PROXIMITY = float(os.getenv("TRIGGER_PROXIMITY", "0.06"))
STEP_INCREMENT_PCT = float(os.getenv("STEP_INCREMENT_PCT", "0.02"))
BASE_TP_PCT = float(os.getenv("BASE_TP_PCT", "2.5"))
BASE_SL_PCT = float(os.getenv("BASE_SL_PCT", "1.6"))

# micro-take profit
MICRO_TP_PCT = float(os.getenv("MICRO_TP_PCT", "1.2"))
MICRO_TP_FRACTION = float(os.getenv("MICRO_TP_FRACTION", "0.6"))
MICRO_MAX_WAIT = float(os.getenv("MICRO_MAX_WAIT", "20.0"))
...
ROLL_TP_STEP_ABS = float(os.getenv("ROLL_TP_STEP_ABS", "0.020"))
ROLL_SL_STEP_ABS = float(os.getenv("ROLL_SL_STEP_ABS", "0.003"))
ROLL_COOLDOWN_SECONDS = int(os.getenv("ROLL_COOLDOWN_SECONDS", "60"))
MAX_ROLLS_PER_POSITION = int(os.getenv("MAX_ROLLS_PER_POSITION", "3"))
ROLL_POST_CANCEL_JITTER = (0.3, 0.8)

# safety/time limits
MAX_HOLD_SECONDS = int(os.getenv("MAX_HOLD_SECONDS", 6 * 3600))  # e.g. 6 hours
DAILY_MAX_DRAWDOWN_USDT = float(os.getenv("DAILY_MAX_DRAWDOWN_USDT", "25.0"))

# REBUY / RECENT BUYS CONFIG
RECENT_BUYS = {}
REBUY_COOLDOWN = int(os.getenv("REBUY_COOLDOWN", 60 * 60))          # 1 hour
LOSS_COOLDOWN = int(os.getenv("LOSS_COOLDOWN", 60 * 60 * 4))        # 4 hours
REBUY_MAX_RISE_PCT = float(os.getenv("REBUY_MAX_RISE_PCT", "5.0"))

# rate-limit/backoff
RATE_LIMIT_BACKOFF = 0
RATE_LIMIT_BACKOFF_MAX = int(os.getenv("RATE_LIMIT_BACKOFF_MAX", "300"))
RATE_LIMIT_BASE_SLEEP = int(os.getenv("RATE_LIMIT_BASE_SLEEP", "90"))
...
# -------------------------
def notify(msg: str, force=False):
    global LAST_NOTIFY
    now_ts = time.time()
    if not force and now_ts - LAST_NOTIFY < 0.9:
        return
    LAST_NOTIFY = now_ts
...
# -------------------------
# SAFE API CALL with backoff
# -------------------------
def safe_api_call(fn, *args, retries=3, backoff=0.6, **kwargs):
    """
    Wraps Binance client calls to handle rate limits and transient errors.
...
def get_price_cached(symbol):
    global TICKER_CACHE, LAST_FETCH, RATE_LIMIT_BACKOFF
    now = time.time()
    # if in backoff use cached tickers
    ...
    if TICKER_CACHE is None or now - LAST_FETCH > CACHE_TTL:
        try:
            ...
            if '-1003' in str(e) or 'Too much request weight' in str(e):
                RATE_LIMIT_BACKOFF = min(RATE_LIMIT_BACKOFF * 2 if RATE_LIMIT_BACKOFF else RATE_LIMIT_BASE_SLEEP,
                                         RATE_LIMIT_BACKOFF_MAX)
                notify(f"⚠️ get_price_cached backing off for {RATE_LIMIT_BACKOFF}s due to rate-limit.")
                time.sleep(min(RATE_LIMIT_BACKOFF, 1.0))
    ...
    # last resort single-symbol ticker
...
def pick_coin():
    ...
    FINAL_CHOICES_LOCAL = globals().get('FINAL_CHOICES', 2)
    MIN_VOL_RATIO_LOCAL = globals().get('MIN_VOL_RATIO', MIN_VOL_RATIO)
    MIN_VOL_RATIO_1M_LOCAL = globals().get('MIN_VOL_RATIO_1M', MIN_VOL_RATIO_1M)
    MIN_OB_LIQ = globals().get('MIN_OB_LIQUIDITY', MIN_OB_LIQUIDITY)
    MIN_OB_IMB = globals().get('MIN_OB_IMBALANCE', MIN_OB_IMBALANCE)
    MAX_OB_SPREAD = globals().get('MAX_OB_SPREAD_PCT', MAX_OB_SPREAD_PCT)
    ...
    for t in tickers:
        ...
        if qvol < MIN_VOLUME:
            continue
        if ch < -5.0 or ch > MAX_24H_RISE_PCT:
            continue
    ...
    for sym, last_price, qvol, change_24h in candidates:
        ...
        # FAST top-of-book pre-check to avoid heavy klines if OB is terrible
        try:
            ob_fast = safe_api_call(client.get_order_book, symbol=sym, limit=2)
            if not ob_fast:
                continue
            ...
            if not bids_f or not asks_f:
                continue
            tbid = float(bids_f[0][0]); task = float(asks_f[0][0])
            spread_fast = (task - tbid) / (tbid + 1e-12) * 100.0
            if spread_fast > max(2.0, MAX_OB_SPREAD * 2.0):
                continue
        except Exception:
            continue
        ...
        try:
            ...
            # require meaningful 5m move and 1m confirmation
            if pct_5m < MIN_5M_PCT or pct_1m < MIN_1M_PCT:
                continue
            if vol_ratio_5m < MIN_VOL_RATIO_LOCAL or vol_ratio_1m < MIN_VOL_RATIO_1M_LOCAL:
                continue
            ...
            if ob_imbalance < MIN_OB_IMB or ob_spread_pct > MAX_OB_SPREAD or bid_quote_liq < MIN_OB_LIQ:
                continue

            # scoring: bias toward strong vol, clear 5m momentum, EMA uplift, and high bid liquidity
            score = 0.0
            score += max(0.0, pct_5m) * 6.0
            score += max(0.0, pct_1m) * 2.5
            score += max(0.0, (vol_ratio_5m - 1.0)) * 5.0 * 100.0
            score += ema_uplift * 10.0 * 100.0
            if rsi_val is not None:
                score += max(0.0, (60.0 - min(rsi_val, 60.0))) * 1.2
            score += math.log1p(qvol) * 0.2
            score += max(0.0, (bid_quote_liq / (MIN_OB_LIQ + 1.0))) * 5.0
            score -= ob_spread_pct * 0.5  # penalize wide spread
    ...
    pool = scored[:min(len(scored), TOP_EVAL_RANDOM_POOL)]
    top_n = min(FINAL_CHOICES_LOCAL, len(pool))
    if top_n <= 0:
        best = scored[0]
    else:
...
# -------------------------
# MARKET BUY helpers (unchanged logic but safe_api_call used)
# -------------------------
...
def place_safe_market_buy(symbol, usd_amount, require_orderbook: bool = False):
    ...
    if require_orderbook:
        try:
            if not orderbook_bullish(symbol, depth=5, min_imbalance=MIN_OB_IMBALANCE, max_spread_pct=MAX_OB_SPREAD_PCT):
    ...
    if order_resp is None:
        notify(f"❌ Market buy failed for {symbol} (API error).")
        RATE_LIMIT_BACKOFF = min(RATE_LIMIT_BACKOFF * 2 if RATE_LIMIT_BACKOFF else RATE_LIMIT_BASE_SLEEP,
                                RATE_LIMIT_BACKOFF_MAX)
        TEMP_SKIP[symbol] = time.time() + max(60, RATE_LIMIT_BACKOFF or RATE_LIMIT_BASE_SLEEP)
        return None, None
...
def place_oco_sell(symbol, qty, buy_price, tp_pct=3.0, sl_pct=1.0,
    ...
    for attempt in range(1, retries + 1):
        ...
        if oco:
            ...
            notify(f"⚠️ OCO SELL attempt {attempt} (standard) failed or rate-limited.")
            if attempt < retries:
                time.sleep(delay)
            else:
                time.sleep(0.2)
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            notify(f"⚠️ OCO SELL attempt {attempt} (alt) failed or rate-limited.")
        time.sleep(0.2)
    ...
    if tp_order:
        ...
        notify(f"❌ Fallback TP limit failed for {symbol}")
    ...
    try:
        ...
        notify(f"❌ Fallback SL stop-limit failed for {symbol}")

    if tp_order or ('sl_order' in locals() and sl_order):
...
# -------------------------
# MONITOR & ROLL (added MAX_HOLD_SECONDS checks)
# -------------------------
        ...
        try:
            ...
            if price_now is None:
                ...
                if res:
                    ...
                    notify("⚠️ Failed to fetch price in monitor (rate-limited). Continuing.")
                    continue
            ...
            # MAX_HOLD_SECONDS emergency exit
            if MAX_HOLD_SECONDS and (time.time() - start_ts) >= MAX_HOLD_SECONDS:
                notify(f"⏱️ MAX_HOLD_SECONDS reached for {symbol}. Forcing market sell fallback.")
                place_market_sell_fallback(symbol, available_for_sell, f)
                return True, price_now, (price_now - entry_price) * orig_qty
            ...
            if ((near_trigger and moved_enough) or rise_trigger_pct or rise_trigger_abs) and available_for_sell >= f.get('minQty', 0.0) and can_roll:
                ...
                if oco2:
                    ...
                    notify("⚠️ Roll attempt failed; will try fallback.")
                    time.sleep(0.4)
...
def get_free_usdt():
    try:
        bal = safe_api_call(client.get_asset_balance, asset=QUOTE)
        if bal:
...
def get_free_asset(asset):
    try:
        bal = safe_api_call(client.get_asset_balance, asset=asset)
        if bal:
            return float(bal.get('free') or 0.0)
    except Exception:
        pass
...
def check_daily_drawdown():
    ...
    if start_balance_usdt is None:
        ...
        return False
    free = get_free_usdt()
    drawdown = start_balance_usdt - free
    if drawdown >= DAILY_MAX_DRAWDOWN_USDT:
        notify(f"🚨 DAILY MAX DRAWDOWN reached (${drawdown:.2f}). Pausing trading for today.")
        return True
    return False
    ...
    while True:
        try:
            if check_daily_drawdown():
                time.sleep(60 * 30)  # pause 30 minutes if drawdown reached
                continue
